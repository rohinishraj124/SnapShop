...
export async function getServerSideProps() {
  ...
  let products = await Product.find({ category: 'innerwear', genre: 'kids' });

  let innerwear = {};

  for (let item of products) {
    if (item.title in innerwear) {
      if (!innerwear[item.title].color.includes(item.color) && item.availableQty > 0) {
        innerwear[item.title].color.push(item.color);
      }
      if (!innerwear[item.title].size.includes(item.size) && item.availableQty > 0) {
        innerwear[item.title].size.push(item.size);
      }
    } else {
      innerwear[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        innerwear[item.title].color = [item.color];
        innerwear[item.title].size = [item.size];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(innerwear)) },
  };
}
