...
const baby = ({ products }) => {
  ...
  return (
    <section className="text-gray-600 body-font bg-gradient-to-b from-white to-gray-100">
      <Head><title>Babycure</title></Head>
...
export async function getServerSideProps() {
  ...
  let products = await Product.find({ category: 'baby', genre: 'kids' });

  let baby = {};

  for (let item of products) {
    if (item.title in baby) {
      if (!baby[item.title].color.includes(item.color) && item.availableQty > 0) {
        baby[item.title].color.push(item.color);
      }
      if (!baby[item.title].size.includes(item.size) && item.availableQty > 0) {
        baby[item.title].size.push(item.size);
      }
    } else {
      baby[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        baby[item.title].color = [item.color];
        baby[item.title].size = [item.size];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(baby)) },
  };
}
