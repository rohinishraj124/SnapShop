...
export async function getServerSideProps() {
  ...
  let products = await Product.find({ category: 'valuepack', genre: 'kids' });

  let valuepack = {};

  for (let item of products) {
    if (item.title in valuepack) {
      if (!valuepack[item.title].color.includes(item.color) && item.availableQty > 0) {
        valuepack[item.title].color.push(item.color);
      }
      if (!valuepack[item.title].size.includes(item.size) && item.availableQty > 0) {
        valuepack[item.title].size.push(item.size);
      }
    } else {
      valuepack[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        valuepack[item.title].color = [item.color];
        valuepack[item.title].size = [item.size];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(valuepack)) },
  };
}
