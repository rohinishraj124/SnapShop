  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          ...
          <div className="flex flex-wrap -m-4">
            {featuredCollections.map((item, index) => (
              <div
                key={index}
                className="sm:w-1/2 md:w-[20em] p-2 w-[11em] m-auto" // Use 'sm:w-1/2' for two columns on small screens
                onMouseEnter={() => setIsHovered(true)}
                onMouseLeave={() => setIsHovered(false)}
              >
                <div className="border border-gray-200 p-2 rounded-lg relative group h-full">
                  {/* Set aspect ratio container to maintain portrait */}
                  <div
                  ...
                  >
                    ...
                    <Image
                      src={
                        isHovered
                          ? item.images[(currentImage.index + 1) % 3]
                          : item.images[currentImage.index]
                      }
                      alt={item.title}
                      layout="fill" // Ensures the image fills the container
                      objectFit="cover" // Ensures the image fills the area and maintains its aspect ratio
                      className="transition-all duration-500 ease-in-out transform group-hover:scale-110 opacity-100 hover:opacity-90"
                    />
                  </div>
                  <h3 className="text-lg text-gray-900 font-medium title-font p-3 mb-2 mt-4">
                    {item.title}
                  </h3>
                </div>
              </div>
        ...
        <div class="flex flex-wrap mt-2 mx-4 mb-4">
          <div class="xl:w-1/3 md:w-1/2 p-4">
            <div class="border border-gray-200 p-6 rounded-lg flex flex-col justify-center items-center ">
              ...
              <p class="leading-relaxed text-base">Our T-Shirts are 100% made of cotton.</p>
            </div>
          </div>
          <div class="xl:w-1/3 md:w-1/2 p-4">
            <div class="border border-gray-200 p-6 rounded-lg flex flex-col justify-center items-center ">
              ...
              <p class="leading-relaxed text-base">We ship all over India for FREE.</p>
            </div>
          </div>
          <div class="xl:w-1/3 md:w-1/2 p-4">
            <div class="border border-gray-200 p-6 rounded-lg flex flex-col justify-center items-center ">
              ...
              <p class="leading-relaxed text-base">We provide amazing offers & discounts on our products.</p>
            </div>
          </div>
        </div>
      </section >
