...
const ethnic = ({ products }) => {
  return (
    <section className="text-gray-600 body-font bg-gradient-to-b from-white to-gray-100">
      <Head><title>Women Ethnic</title></Head>
...
export async function getServerSideProps() {
  ...
  let products = await Product.find({ category: 'ethnic', genre: 'women' });

  let ethnic = {};

  for (let item of products) {
    if (item.title in ethnic) {
      if (!ethnic[item.title].color.includes(item.color) && item.availableQty > 0) {
        ethnic[item.title].color.push(item.color);
      }
      if (!ethnic[item.title].size.includes(item.size) && item.availableQty > 0) {
        ethnic[item.title].size.push(item.size);
      }
    } else {
      ethnic[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        ethnic[item.title].color = [item.color];
        ethnic[item.title].size = [item.size];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(ethnic)) },
  };
}
