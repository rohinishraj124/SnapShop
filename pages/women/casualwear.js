...
const casual = ({ products }) => {
  return (
    ...
    <section className="text-gray-600 body-font bg-gradient-to-b from-white to-gray-100">
      <Head><title>Women Casual Wear</title></Head>
...
export async function getServerSideProps() {
  ...
  let products = await Product.find({ category: 'casual', genre: 'women' });

  let casual = {};

  for (let item of products) {
    if (item.title in casual) {
      if (!casual[item.title].color.includes(item.color) && item.availableQty > 0) {
        casual[item.title].color.push(item.color);
      }
      if (!casual[item.title].size.includes(item.size) && item.availableQty > 0) {
        casual[item.title].size.push(item.size);
      }
    } else {
      casual[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        casual[item.title].color = [item.color];
        casual[item.title].size = [item.size];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(casual)) },
  };
}
