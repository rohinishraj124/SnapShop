...
const western = ({ products }) => {
  ...
  return (
    <section className="text-gray-600 body-font bg-gradient-to-b from-white to-gray-100">
      <Head><title>Women Western</title></Head>
...
export async function getServerSideProps() {
  ...
  let products = await Product.find({ category: 'western', genre: 'women' });

  let western = {};

  for (let item of products) {
    if (item.title in western) {
      if (!western[item.title].color.includes(item.color) && item.availableQty > 0) {
        western[item.title].color.push(item.color);
      }
      if (!western[item.title].size.includes(item.size) && item.availableQty > 0) {
        western[item.title].size.push(item.size);
      }
    } else {
      western[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        western[item.title].color = [item.color];
        western[item.title].size = [item.size];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(western)) },
  };
}
