...
const jewellery = ({ products }) => {
  ...
  return (
    <section className="text-gray-600 body-font bg-gradient-to-b from-white to-gray-100">
      <Head><title>Women Jewellery</title></Head>
            ...
            <Link key={products[product]._id} href={`/product/${products[product].slug}`}>
              <div className="cursor-pointer bg-white shadow-md rounded-lg p-4 hover:shadow-xl hover:scale-105 transform transition duration-300 ease-in-out dark:bg-gray-800">
                ...
                <div className="relative h-48 rounded overflow-hidden">
                  <img
                    alt="ecommerce"
                    className="m-auto h-[30vh]  block object-cover object-bottom"
...
export async function getServerSideProps() {
  ...
  let products = await Product.find({ category: 'jewellery', genre: 'women' });

  let jewellery = {};

  for (let item of products) {
    if (item.title in jewellery) {
      if (!jewellery[item.title].color.includes(item.color) && item.availableQty > 0) {
        jewellery[item.title].color.push(item.color);
      }
      if (!jewellery[item.title].size.includes(item.size) && item.availableQty > 0) {
        jewellery[item.title].size.push(item.size);
      }
    } else {
      jewellery[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        jewellery[item.title].color = [item.color];
        jewellery[item.title].size = [item.size];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(jewellery)) },
  };
}
